...
A lead is considered matched when:

- Email matches and the name looks like the same person
- Phone matches and the name looks like the same person
- Both phone and email match

It flags a lead for manual review when:

- There is no phone or email match in GHL
- Email matches, but the name does not look like the same person
- Phone matches, but the name does not look like the same person

The name does not need to be exact. The tool accepts clear same-person formats like `Maria Garcia` vs `Maria G.` or `John A. Smith` vs `John Smith`.

Default name similarity threshold: `70%`
