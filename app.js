...
function nameSimilarity(a, b) {
  ...
  return Math.max(
    0,
    Math.round(((longer.length - editDistance(longer, shorter)) / longer.length) * 1000) / 10
  );
}
...
function tokensCompatible(a, b) {
  ...
  // Initial match, like "G" and "Garcia"
  if (a.length === 1 && b.startsWith(a)) return true;
  if (b.length === 1 && a.startsWith(b)) return true;

  // Shortened first names, like "Rob" and "Robert"
...
function namesLookLikeSamePerson(metaName, ghlName, threshold) {
  const meta = normalizeName(metaName);
  const ghl = normalizeName(ghlName);

  if (!meta || !ghl) {
    return {
      isMatch: false,
      score: 0,
      reason: "Name missing on one side"
    };
  }

  const score = nameSimilarity(meta, ghl);
  ...
  if (score >= threshold) {
    return {
      isMatch: true,
      score,
      reason: "Name is similar enough"
    };
  }

  const metaTokens = getNameTokens(meta);
  const ghlTokens = getNameTokens(ghl);

  const metaFirst = metaTokens[0] || "";
  const ghlFirst = ghlTokens[0] || "";
  const metaLast = metaTokens.length > 1 ? metaTokens[metaTokens.length - 1] : "";
  const ghlLast = ghlTokens.length > 1 ? ghlTokens[ghlTokens.length - 1] : "";

  const firstCompatible = tokensCompatible(metaFirst, ghlFirst);
  const lastCompatible = metaLast && ghlLast ? tokensCompatible(metaLast, ghlLast) : true;

  // Handles:
  // Maria Garcia vs Maria G.
  ...
  if (firstCompatible && lastCompatible) {
    return {
      isMatch: true,
      score,
      reason: "Name format looks like the same person"
    };
  }

  const metaSet = new Set(metaTokens);
  const ghlSet = new Set(ghlTokens);
  const overlap = [...metaSet].filter((token) => ghlSet.has(token)).length;
  const smallerTokenCount = Math.min(metaSet.size, ghlSet.size);

  if (smallerTokenCount > 0 && overlap / smallerTokenCount >= 0.6) {
    return {
      isMatch: true,
      score,
      reason: "Most name parts overlap"
    };
  }

  // Handles reversed names, like Smith John vs John Smith
  const sortedMeta = [...metaTokens].sort().join(" ");
  const sortedGhl = [...ghlTokens].sort().join(" ");
  if (sortedMeta && sortedMeta === sortedGhl) {
    return {
      isMatch: true,
      score,
      reason: "Same name parts in different order"
    };
  }
  ...
  return {
    isMatch: false,
    score,
    reason: "Name does not look like the same person"
  };
}
...
function findBestMatch(metaRow, ghlRows, nameThreshold) {
  ...
  candidates.forEach((candidate) => {
    const phoneMatch = Boolean(
      metaRow._normalized_phone && metaRow._normalized_phone === candidate._normalized_phone
    );
    const emailMatch = Boolean(
      metaRow._normalized_email && metaRow._normalized_email === candidate._normalized_email
    );

    const nameCheck = namesLookLikeSamePerson(
      metaRow._display_name,
      candidate._display_name,
      nameThreshold
    );
    ...
    let isMatched = false;
    let reason = "";
    let identifierRank = 0;

    if (phoneMatch && emailMatch) {
      matchType = "Phone + email";
      identifierRank = 3000;
      isMatched = true;
      reason = nameCheck.isMatch
        ? `Phone + email match, ${nameCheck.reason.toLowerCase()}`
        : "Phone + email match";
    } else if (emailMatch) {
      matchType = "Email only";
      identifierRank = 2000;
      isMatched = nameCheck.isMatch;
      reason = nameCheck.isMatch
        ? `Email match, ${nameCheck.reason.toLowerCase()}`
        : "Email matches, but name does not look like the same person";
    } else if (phoneMatch) {
      matchType = "Phone only";
      identifierRank = 1900;
      isMatched = nameCheck.isMatch;
      reason = nameCheck.isMatch
        ? `Phone match, ${nameCheck.reason.toLowerCase()}`
        : "Phone matches, but name does not look like the same person";
    }

    const rank = identifierRank + (nameCheck.isMatch ? 500 : 0) + nameCheck.score;
    ...
    if (rank > bestRank) {
      ...
      bestResult = {
        ...
        nameLooksSame: nameCheck.isMatch,
        matchType,
        isMatched
      };
    }
...
function runMatcher(usingOverride) {
  try {
    ...
    metaRows.forEach((metaRow) => {
      ...
      const needsReview = !result.isMatched;
      ...
      const outputRow = {
        ...
        name_looks_like_same_person: result.nameLooksSame ? "YES" : "NO",
...
function renderReviewTable(rows) {
  ...
  const compactRows = previewRows.map((row) => ({
    ...
    name_looks_like_same_person: row.name_looks_like_same_person,
